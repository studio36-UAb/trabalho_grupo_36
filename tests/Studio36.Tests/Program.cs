using Studio36.Tests;

List<TestCase> tests = new()
{
    new("T01", "Apresentacao do menu inicial", T01_MenuInicial.Run),
    new("T02", "Terminar aplicacao pela opcao 3", T02_TerminarOpcao3.Run),
    new("T03", "Login com credenciais validas", T03_LoginValido.Run),
    new("T04", "Login com credenciais invalidas", T04_LoginInvalido.Run),
    new("T05", "Acesso a opcao de registo", T05_AcessoRegisto.Run),
    new("T06", "Opcao invalida no menu", T06_OpcaoInvalida.Run),
    new("T07", "Input nao numerico no menu", T07_InputNaoNumerico.Run),
    new("T08", "Input vazio no menu", T08_InputVazio.Run),
    new("T09", "Model valida credenciais validas", T09_ModelLoginValido.Run),
    new("T10", "Model rejeita credenciais invalidas", T10_ModelLoginInvalido.Run),
    new("T18", "Criar projeto sem nome", T18_CriarProjetoSemNome.Run),
    new("T19", "Criar projeto com datas invalidas", T19_CriarProjetoComDatasInvalidas.Run),
    new("T26", "Listar tarefas de projeto inexistente", T26_ProjectNotFound.Run),
    new("T27", "Introduzir ID de projeto nao numerico na listagem de tarefas", T27_ProjectIdNaoNumerico.Run),
    new("T28", "Criar projeto valido", T28_CriarProjetoValido.Run),
    new("T29", "Listar projetos", T29_ListarProjetos.Run),
    new("T30", "Editar projeto valido", T30_EditarProjetoValido.Run),
    new("T31", "Controller depende de IModel e IView", T31_ControllerDependeDeInterfaces.Run),
    new("T32", "Controller funciona com implementacoes fake das interfaces", T32_ControllerFuncionaComFakes.Run),
    new("T33", "Fluxo MVC completo na criacao de projeto", T33_FluxoCriacaoProjetoMvc.Run),
    new("T34", "Fluxo MVC completo na listagem de projetos", T34_FluxoListagemProjetosMvc.Run),
    new("T35", "Fluxo MVC completo na edicao de projeto valido", T35_FluxoEditarProjetoValidoMvc.Run),
    new("T36", "Fluxo MVC na edicao de projeto inexistente", T36_FluxoEditarProjetoInexistenteMvc.Run),
    new("T37", "Fluxo MVC na edicao de projeto com nome vazio", T37_FluxoEditarProjetoNomeVazioMvc.Run),
    new("T38", "Eliminar projeto valido", T38_EliminarProjetoValido.Run),
    new("T39", "Fluxo MVC completo na geracao de relatorio", T39_FluxoGerarRelatorioMvc.Run),
    new("T40", "Gerar relatorio valido", T40_GerarRelatorioValido.Run),
    new("T41", "Gerar relatorio de projeto inexistente", T41_GerarRelatorioProjetoInexistente.Run)
};

if (args.Length > 0)
{
    HashSet<string> requestedIds = args
        .Select(arg => arg.Trim().ToUpperInvariant())
        .Where(arg => !string.IsNullOrWhiteSpace(arg))
        .ToHashSet();

    tests = tests
        .Where(test => requestedIds.Contains(test.Id))
        .ToList();

    if (tests.Count == 0)
    {
        Console.WriteLine($"No matching tests found for: {string.Join(", ", requestedIds)}");
        Environment.Exit(1);
    }
}

int failed = 0;
List<string> passedTests = new();
List<string> failedTests = new();

foreach (TestCase test in tests)
{
    try
    {
        test.Run();
        passedTests.Add($"{test.Id} - {test.Description}");
    }
    catch (Exception ex)
    {
        failed++;
        failedTests.Add($"{test.Id} - {test.Description}: {ex.Message}");
    }
}

Console.WriteLine("\n========================================");
Console.WriteLine("TEST SUMMARY");
Console.WriteLine("========================================");
Console.WriteLine($"Total: {tests.Count} | Passed: {passedTests.Count} | Failed: {failedTests.Count}");

if (passedTests.Count > 0)
{
    Console.WriteLine("\nPassed tests:");
    foreach (var test in passedTests)
    {
        Console.WriteLine($"  ✓ {test}");
    }
}

if (failedTests.Count > 0)
{
    Console.WriteLine("\nFailed tests:");
    foreach (var test in failedTests)
    {
        Console.WriteLine($"  ✗ {test}");
    }
}

Console.WriteLine("========================================");

if (failed > 0)
{
    Console.WriteLine($"\n{failed} test(s) failed.");
    Environment.Exit(1);
}

Console.WriteLine("\n✓ All tests passed!");
